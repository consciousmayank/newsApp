...
class TopHeadlinesFilterWidget extends ViewModelWidget<NewsListViewModel> {
  ...
  Widget build(BuildContext context, NewsListViewModel viewModel) {
    return PopupMenuButton(
      ...
      itemBuilder: (context) {
        return List.generate(viewModel.topHeadlinesfilterOptions.length,
            (index) {
          return PopupMenuItem(
            value: index,
            child: Text(
              viewModel.topHeadlinesfilterOptions[index],
            ),
          );
        });
      },
      onSelected: viewModel.handleTopHeadlinesFilterOptionSelection,
    );
  }
}
